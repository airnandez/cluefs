...
func validateFormat(csv, json bool) (string, error) {
	if csv && json {
...
DESCRIPTION:
{{.Sp3}}{{.AppName}} mounts a synthetic file system which purpose is to generate
...
{{.Sp3}}to the target file system, that is, the one which hosts the shadow directory.
{{.Sp3}}See the EXAMPLES section below.
...
{{.Tab1}}CSV is the default output format.
